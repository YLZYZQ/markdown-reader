...
const DEFAULT_PREFERENCES = Object.freeze({
  theme: 'system',    // 'system' | 'light' | 'cream' | 'dark'
  ...
  updateCheckEnabled: true, // 启动后自动检查 GitHub 稳定发布
  lastUpdateCheckAt: 0,   // 上次成功检查时间戳 ms
  menuLanguage: 'zh-CN',  // 应用菜单语言 'zh-CN' | 'en-US'
});

const THEME_VALUES = new Set(['system', 'light', 'cream', 'dark']);
const MENU_LANGUAGE_VALUES = new Set(['zh-CN', 'en-US']);
...
function sanitizePreferences(raw) {
  ...
  prefs.updateCheckEnabled = source.updateCheckEnabled !== false;
  prefs.lastUpdateCheckAt = clampNumber(
    source.lastUpdateCheckAt,
    0,
    { min: 0, max: Number.MAX_SAFE_INTEGER, integer: true },
  );
  if (MENU_LANGUAGE_VALUES.has(source.menuLanguage)) prefs.menuLanguage = source.menuLanguage;
